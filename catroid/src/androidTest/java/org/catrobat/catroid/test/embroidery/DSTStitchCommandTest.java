...
public class DSTStitchCommandTest {
	...
	public void testAddSimpleStitchCommand() {
		...
		verify(stream, times(1)).addStitchPoint(eq(xCoord), eq(yCoord), Color.BLACK);
	}
	...
	public void testActDuplicateStitchCommand() {
		...
		verify(stream, Mockito.never()).addStitchPoint(anyFloat(), anyFloat(), Color.BLACK);
	}
	...
	public void testSpriteTriggeredColorChange() {
		...
		verify(stream, times(2)).addStitchPoint(eq(0.0f), eq(0.0f), Color.BLACK);
		verify(stream, times(1)).addStitchPoint(eq(xCoord), eq(yCoord), Color.BLACK);
	}
	...
	public void testLayerSwitchTriggeredColorChange() {
		...
		inOrder.verify(stream, times(1)).addStitchPoint(eq(1.0f), eq(1.0f), Color.BLACK);
		inOrder.verify(stream, times(1)).addJump();
		inOrder.verify(stream, times(2)).addStitchPoint(eq(1.0f), eq(1.0f), Color.BLACK);
		inOrder.verify(stream, times(1)).addStitchPoint(eq(xCoord), eq(yCoord), Color.BLACK);
	}
	...
	public void testAddPreviousCommandOfSpriteOfOtherLayer() {
		...
		verify(stream, times(1)).addStitchPoint(eq(1.0f), eq(1.0f), Color.BLACK);
		verify(stream, times(1)).addStitchPoint(eq(xCoord), eq(yCoord), Color.BLACK);
	}
